...
class auditd::config {
  ...
  # rules in /etc/audit/rules.d. augenrules rewrites them (mode 0640) each
  # time auditd loads its rules, so enforcing $config_file_mode (0600) here
  # fights augenrules: Puppet resets the mode every run, which refreshes
  # auditd::config -> restarts the service -> regenerates the files ->
  # repeat, breaking idempotency (observed on EL8). Leave the mode to
  # augenrules; 0640 satisfies the STIG requirement for these files (0640 or
  # less permissive). The explicit declaration still shields them from the
  # recurse/purge on /etc/audit.
  file { [
