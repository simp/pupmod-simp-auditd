  ...
  hosts.each do |host|
    context "on #{host}" do
      ...
      context 'default parameters' do
        ...
        it 'does not fight augenrules over /etc/audit/audit.rules permissions' do
          # /etc/audit/audit.rules is generated by augenrules, which rewrites
          # it as mode 0640 every time auditd loads its rules. If Puppet
          # reverts that mode it refreshes auditd::config, restarts the
          # service, regenerates the file, and repeats on every run -- a
          # non-idempotent loop that only surfaces once augenrules has set the
          # file back to 0640 (which the single idempotency apply above does
          # not deterministically exercise). Simulate augenrules' 0640 and
          # confirm a subsequent apply stays idempotent and leaves it alone.
          on(host, 'chmod 0640 /etc/audit/audit.rules')
          apply_manifest_on(host, manifest, catch_changes: true)
