...
describe 'auditd class with sample rulesets' do
  ...
  hosts_with_role(hosts, 'el8').each do |host|
    ...
    context "on #{host}" do
      # The privileged ruleset is generated by execs on the first real apply,
      # so previewing it before then is the case noop has to survive.
      context 'with privileged sample ruleset in noop mode before any apply' do
